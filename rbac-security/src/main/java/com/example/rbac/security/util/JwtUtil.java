...
import com.example.rbac.security.config.JwtConfig;
...
public class JwtUtil {
    ...
    /**
     * The signing key, derived from jwt.config.key. jjwt 0.12 dropped the old
     * {@code signWith(SignatureAlgorithm, String)} form in favour of a real {@link SecretKey}, and
     * rejects anything shorter than 256 bits for HS256 — hence the length check on the property.
     */
    private SecretKey signingKey() {
        byte[] keyBytes = jwtConfig.getKey().getBytes(StandardCharsets.UTF_8);
    ...
    public String createJWT(Boolean rememberMe, Long id, String subject, List<String> roles, Collection<? extends GrantedAuthority> authorities) {
        Date now = new Date();
        JwtBuilder builder = Jwts.builder().id(id.toString()).subject(subject).issuedAt(now).signWith(signingKey()).claim("roles", roles).claim("authorities", authorities);

        // 设置过期时间
        Long ttl = rememberMe ? jwtConfig.getRemember() : jwtConfig.getTtl();
        if (ttl > 0) {
            builder.expiration(new Date(now.getTime() + ttl));
        }
    ...
    public Claims parseJWT(String jwt) {
        try {
            ...
            if (!Objects.equals(jwt, redisToken)) {
