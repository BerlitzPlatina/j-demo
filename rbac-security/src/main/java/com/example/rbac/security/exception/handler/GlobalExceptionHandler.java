package com.example.rbac.security.exception.handler;
...
public class GlobalExceptionHandler {
    ...
    public ApiResponse handlerException(Exception e) {
        if (e instanceof NoHandlerFoundException) {
            ...
            // hutool's CollUtil.getFirst returned null for an empty set, which would have thrown a
            // NullPointerException here; fall back to the generic message instead.
            String violationMessage = ((ConstraintViolationException) e).getConstraintViolations().stream().findFirst().map(ConstraintViolation::getMessage).orElse(Status.BAD_REQUEST.getMessage());
            return ApiResponse.of(Status.BAD_REQUEST.getCode(), violationMessage, null);
