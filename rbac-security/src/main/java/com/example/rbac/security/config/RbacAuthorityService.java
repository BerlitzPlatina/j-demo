...
public class RbacAuthorityService {
    ...
    public boolean hasPermission(HttpServletRequest request, Authentication authentication) {
        ...
        if (userInfo instanceof UserDetails) {
            ...
            List<Permission> btnPerms = permissions.stream()
                ...
                // 过滤 METHOD 为空
                .filter(permission -> StringUtils.hasText(permission.getMethod())).collect(Collectors.toList());

            for (Permission btnPerm : btnPerms) {
                RequestMatcher antPathMatcher = PathPatternRequestMatcher.withDefaults().matcher(HttpMethod.valueOf(btnPerm.getMethod()), btnPerm.getUrl());
    ...
    private void checkRequest(HttpServletRequest request) {
        ...
        for (String uri : urlMapping.keySet()) {
            ...
            RequestMatcher antPathMatcher = PathPatternRequestMatcher.withDefaults().matcher(uri);
            if (antPathMatcher.matches(request)) {
    ...
    private Multimap<String, String> allUrlMapping() {
        ...
        handlerMethods.forEach((k, v) -> {
            // 获取当前 key 下的获取所有URL
            // Spring 6 matches with PathPattern rather than the old Ant patterns, so the patterns
            // live in getPathPatternsCondition(); getPatternsCondition() is null here.
            Set<String> url = k.getPathPatternsCondition().getPatternValues();
